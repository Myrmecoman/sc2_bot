...
def handle_refineries(self : BotAI, step: int):
    ...
    # handle workers - during an early rush, cap gas at 1 worker/refinery instead of fully
    # saturating it, so the rest mine minerals instead: minerals fund Marines, our primary
    # defense, faster than a fully-saturated refinery does anything for us right now
    under_rush: bool = self.worker_rushed or self.army_advisor.zergling_rushed
    for r in self.gas_buildings.ready:
        target_harvesters = 1 if under_rush else r.ideal_harvesters
        if r.assigned_harvesters < target_harvesters and step - self.refineries_age[r.tag] > 6: # last check because when it is finished there are 0 workers altough the one building goes to it instantly
            ...
            if workers:
                for w in workers:
                    if not w.is_carrying_minerals and not w.is_carrying_vespene:
                        w.gather(r)
                        return
        if r.assigned_harvesters > target_harvesters or self.workers.amount <= 6:
