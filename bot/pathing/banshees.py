...
class Banshees:
    def __init__(self, ai: BotAI, pathing: Pathing):
        ...
        # the 4 closest expansions to the enemy's main, closest first - that's their own main
        # (distance ~0), then natural, 3rd, 4th, matching the "1st base, 2nd, 3rd, 4th" comment on
        # get_free_roaming_spot below. Previously all 4 slots were hardcoded to enemy_start_locations[0]
        # (the same single point, 4 times over), so every roaming banshee converged on the enemy's
        # main regardless of which slot it was assigned - the whole point of spreading harass
        # across 4 different bases was silently defeated
        self.roam_spot_positions = sorted(
            self.ai.expansion_locations_list,
            key=lambda loc: loc.distance_to(self.ai.enemy_start_locations[0]),
        )[:4]
