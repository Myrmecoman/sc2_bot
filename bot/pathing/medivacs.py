...
class Medivacs:
    ...
    async def retreat_to(self, units: Units, pos: Point2):
        """
        Path to pos - but retreat from danger first, same as handle_attackers already does. A
        Medivac can't fight back, so sitting still in a dangerous spot while "holding" was never
        useful; this was the one place that check was missing.
        """
        grid = self.pathing.air_grid
        for unit in units:
