...
try:
    # map_analyzer needs its compiled C extension (mapanalyzerext) for the platform/Python/NumPy
    # combination actually running - missing, wrong-platform, or ABI-mismatched binaries have all
    # happened before on AI Arena's Linux servers. Import failure is caught here (not left to crash
    # the whole bot at startup) so bot.py can fall back to the pure-Python implementation instead.
