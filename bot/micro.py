...
SCOUT_BASES_HOLD_DURATION = 30.0  # how long a go_scout_bases sweep is protected from the army manager - long enough to actually reach and check an expansion, short enough to react if something real happens elsewhere
DETACHMENT_RESPONSE_RADIUS = 15.0   # enemies within this of the attack position count as "the specific thing we're responding to" for sizing a detachment
DETACHMENT_POWER_RATIO = 2.5        # power edge a detachment should have over that specific threat - a real margin, but nowhere near needing the whole army's worth of overkill
DETACHMENT_MIN_SIZE = 4             # always send at least this many, so a weak/empty target still gets dealt with promptly instead of waiting on a bigger commitment
SMALL_THREAT_ARMY_FRACTION = 0.5    # once the detachment needed would exceed this fraction of bio, it's not "a few units" anymore - commit the whole grouped force instead of trickling in piecemeal
...
def size_bio_detachment(self : BotAI, bio : Units, pos : Point2, enemies : Units) -> Optional[Units]:
    """Which bio units are actually needed to deal with the specific enemy presence at pos - not
    the whole grouped army. A handful of enemy units (or an undefended base with nothing but
    buildings left) doesn't need our entire force marching across the map for it, leaving nothing
    to answer anywhere else in the meantime; the whole army is for large, planned attacks. Picks
    the closest bio first and stops once it has a real power margin over the threat (or the
    DETACHMENT_MIN_SIZE floor, whichever needs more), so a weak/empty target still gets a prompt
    response rather than waiting on an arbitrarily large one. Returns None when the threat is
    big enough that splitting doesn't make sense - just commit the whole grouped force instead."""
    threat: Units = enemies.closer_than(DETACHMENT_RESPONSE_RADIUS, pos)
    threat_power: float = sum(self.army_advisor.unit_power(u) for u in threat)
    needed_power: float = threat_power * DETACHMENT_POWER_RATIO

    sorted_bio: Units = bio.sorted_by_distance_to(pos)
    detachment_tags: Set[int] = set()
    running_power = 0.0
    for u in sorted_bio:
        if running_power >= needed_power and len(detachment_tags) >= DETACHMENT_MIN_SIZE:
            break
        detachment_tags.add(u.tag)
        running_power += self.army_advisor.unit_power(u)

    if len(detachment_tags) >= bio.amount * SMALL_THREAT_ARMY_FRACTION:
        return None
    return bio.tags_in(detachment_tags)
...
def go_scout_bases(self : BotAI) -> bool:
    """Returns True only when it actually issued new sweep orders (not still on cooldown) - the
    caller uses that to know whether to hold the army manager off the units it just dispatched.
    Also records which units those were and for how long (self.scouting_unit_tags/
    scouting_bases_until), so micro() can keep excluding them from the normal per-unit-class
    dispatch afterwards instead of just this one step."""
    ...
    for i in self.expansion_locations:
        if counter >= ground_units.amount:
            break
        unit = ground_units[counter]
        unit.attack(i)
        unit.attack(i.towards(self.game_info.map_center, -9), True)
        dispatched.add(unit.tag)
        counter += 1
    ...
    self.scouting_unit_tags = dispatched
    self.scouting_bases_until = self.time + SCOUT_BASES_HOLD_DURATION
    return True
...
async def micro(self : BotAI):
    ...
    # units go_scout_bases just dispatched keep executing their sweep undisturbed for a while
    # instead of being handed straight back to their unit class's own handle_attackers/retreat_to
    # the very next step, which would otherwise immediately redirect them back to the rally
    # point/attack target before they'd covered any ground - tags_not_in on an empty set is a
    # harmless no-op, so this applies unconditionally rather than needing its own branch below
    scouting_tags: Set[int] = self.scouting_unit_tags if self.time < self.scouting_bases_until else set()

    # always attack with reapers and banshees
    await self.reapers.handle_attackers(self.units(UnitTypeId.REAPER).tags_not_in(scouting_tags))
    await self.banshees.handle_attackers(self.units(UnitTypeId.BANSHEE).tags_not_in(scouting_tags))
    ...
    units : Units = self.units.exclude_type({
        ...
        UnitTypeId.LIBERATOR, UnitTypeId.LIBERATORAG}).tags_not_in(scouting_tags)
    if self.army_count == 0:
        return

    # before 5 minutes an empty-looking enemy main is almost always just "haven't actually looked
    # yet", not a hidden base - and Terran doesn't hide expansions the way Zerg/Protoss can, so
    # there's no point fanning the army out across every expansion hunting for one
    if are_we_idle_at_enemy_base(self) and self.time >= 300 and self.enemy_race in {Race.Zerg, Race.Protoss}:
        # go_scout_bases has its own 90s internal cooldown and returns False (did nothing) while
        # on it - only bail out of the rest of army management on the step it actually dispatched
        # new orders. Unconditionally returning here regardless of that would otherwise freeze
        # EVERY unit's handle_attackers/retreat_to for as long as this trigger condition holds
        # (which can be a very long time), not just the units actually sent scouting
        if go_scout_bases(self):
            return

    # the check above only fires while a unit is standing right at the enemy's (now-empty) main -
    # once we've actually razed every building we knew about and the army moves on or regroups,
    # it stops being true and this could otherwise just... stop looking forever. That's exactly
    # how a game gets tied: we win the fight, raze the main, and a hidden base sits uncontested
    # for the rest of the clock because nothing ever goes looking for it again. If we've got no
    # known enemy structure left AND nothing better to spend supply on (maxed out), there's
    # nothing to lose by fanning out to hunt - not race-gated like the check above, since a
    # hidden base is a hidden base regardless of how the enemy pulled it off (Terran can still
    # tuck a lone base away, e.g. by flying a Command Center out - see prevent_PF_rush)
    if self.enemy_structures.amount == 0 and self.supply_left <= 0:
        if go_scout_bases(self):
            return
    ...
    bio : Units = self.units.of_type({UnitTypeId.MARINE, UnitTypeId.MARAUDER}).tags_not_in(scouting_tags)
    medivacs : Units = self.units(UnitTypeId.MEDIVAC).tags_not_in(scouting_tags)
    ravens : Units = self.units(UnitTypeId.RAVEN).tags_not_in(scouting_tags)
    flying_vikings : Units = self.units(UnitTypeId.VIKINGFIGHTER).tags_not_in(scouting_tags)
    tanks : Units = self.units.of_type({UnitTypeId.SIEGETANK, UnitTypeId.SIEGETANKSIEGED}).tags_not_in(scouting_tags)
    cyclones : Units = self.units(UnitTypeId.CYCLONE).tags_not_in(scouting_tags)
    liberators : Units = self.units.of_type({UnitTypeId.LIBERATOR, UnitTypeId.LIBERATORAG}).tags_not_in(scouting_tags)
    ...
    if attack:
        ...
        # a real, planned attack commits the whole grouped army - but should_attack can also go
        # True just because a handful of enemy units are standing somewhere weak enough that our
        # power ratio clears easily on its own. Sending the entire force after a few units leaves
        # nothing covering anywhere else and is needless overkill: split off just enough bio
        # instead, and let the rest hold near the anchor (joins the stragglers pile above) rather
        # than also marching off after something that doesn't need them. Tanks/Cyclones go with
        # the reserve too when this triggers - they're slow, standing-army pieces, not a
        # fast-response force, so they're not worth committing to a target this small either.
        # Only meaningful when pos is an actual known enemy presence, not the blind guess at their
        # start location we fall back to with no enemies visible at all - there's no "threat" to
        # size a response to in that case
        if enemies.amount > 0:
            detachment: Optional[Units] = size_bio_detachment(self, bio, pos, enemies)
            if detachment is not None:
                bio_stragglers = bio_stragglers | bio.tags_not_in(detachment.tags)
                bio = detachment
                tanks_stragglers = tanks_stragglers | tanks
                tanks = tanks.tags_not_in(tanks.tags)
                cyclones_stragglers = cyclones_stragglers | cyclones
                cyclones = cyclones.tags_not_in(cyclones.tags)
        ...
        await self.liberators.handle_attackers(liberators, pos)
        if diversion_squad:
        ...
        await self.liberators.retreat_to(liberators, pos)
