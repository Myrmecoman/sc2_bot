...
import numpy as np
from training_bots.MassReaper.consts import ALL_STRUCTURES
from training_bots.MassReaper.influence_costs import INFLUENCE_COSTS
...
try:
    # see bot/pathing/pathing.py for why this is guarded - a missing/wrong-platform/ABI-mismatched
    # mapanalyzerext must not crash the whole script at import time (run.py imports every training
    # bot unconditionally at startup, including this one, regardless of which bot is actually playing)
...
class Pathing:
    def __init__(self, ai: BotAI, debug: bool) -> None:
        if MapData is None:
            raise RuntimeError("map_analyzer's compiled C extension is unavailable on this platform")
